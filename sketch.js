...
function setup() {
  ...
  lock = createSprite(400,450);
  lock.addImage(lockImage)
  lock.scale = 0.2
  
  lock1 = createSprite(650,450);
...
function draw() {
  ...
  if (gameState === "pause") {
   ...
   if(b < 25){
    text("Unlock with a highscore of ",330,505);
    text("250 or above",365,520)
   }
 
   if(b < 50){
    text("Unlock with a highscore of ",580,505);
    text("500 or above",615,520)
   }
